# Lesson 9: Basic debugging in IDE
...
### 7. Debugging Strategy (Стратегія пошуку помилок)

Коли щось не працює:

1. **Прочитайте повідомлення про помилку** — traceback читається знизу вверх
2. **Відтворіть помилку** — знайдіть мінімальний вхід, який її викликає
3. **Звузьте область пошуку** — додайте print() або breakpoint у середині функції
4. **Перевірте припущення** — подивіться на реальні значення змінних, а не те що ви очікуєте
5. **Виправте та перевірте** — після фіксу запустіть код з тим самим вхідним значенням

**Порада:** Більшість помилок — це не в тому що код складний, а в тому що дані не такі як ви думали.

---

### 8. Типові помилки під час дебагу

| Помилка | Як уникнути |
|---------|-------------|
| Дебажити не той файл | Перевірте який файл запущено |
| Забути зберегти файл перед Debug | Ctrl+S перед кожним запуском |
| Breakpoint стоїть, але не спрацьовує | Перевірте чи код дійсно проходить через цей рядок |
| Виправив помилку, але не перезапустив | Debug завжди запускає код заново |
