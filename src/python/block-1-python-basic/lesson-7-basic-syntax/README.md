...
```python
#  В прикладі змінна `x` спочатку має значення 10. 
#  Потім ми використовуємо оператори присвоювання, щоб змінити її значення на основі поточного, (тобто `x` оновлюється кожного разу):
x = 10
