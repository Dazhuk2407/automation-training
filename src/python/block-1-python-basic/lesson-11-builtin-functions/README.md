...
Python має багато вбудованих функцій. У цьому уроці ми вивчимо найуживаніші з них.
...
### 7. Коли яку функцію використовувати

| Задача | Використовуй | Замість |
|--------|--------------|---------|
| Потрібен індекс у циклі | `enumerate(items)` | `for i in range(len(items))` |
| Пройти два списки разом | `zip(names, ages)` | Ручний доступ по індексу |
| Перевірити ВСІ елементи | `all(x > 0 for x in nums)` | Цикл з прапорцем |
| Перевірити хоча б один | `any(x > 0 for x in nums)` | Цикл з break |
| Відсортувати без зміни оригіналу | `sorted(lst)` | `lst.sort()` (змінює in-place) |
| Отримати новий список | `sorted(lst)` | `lst.sort()` нічого не повертає |
